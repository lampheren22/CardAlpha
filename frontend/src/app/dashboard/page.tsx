...
import type { CardListItem, Filters, DashboardSummary } from '@/lib/types'
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            {summaryLoading ? 'Loading…' : isError ? 'Could not reach API' : `${displaySummary.total_analyzed} cards analyzed · ${displaySummary.buy_signals} buy signals`}
          </p>
        </div>
      </div>
            ...
            {SPORT_FILTERS.map((s) => (
              <button key={s} onClick={() => setSportFilter(s)}
                className={clsx('px-3 py-1.5 rounded-lg text-sm font-medium transition-colors',
                  sportFilter === s ? 'bg-alpha-green text-black' : 'text-gray-400 hover:text-white'
                )}>
                {s}
              </button>
            ))}
